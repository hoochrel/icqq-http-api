...
function botLogin() {
    ...
    try {
        ...
        bot.login(account, password.length ? password : null);
    } catch {
        inputPassword();
    }
}
 ...
 */
function createBot() {
    bot = oicq.createClient({log_level:config.log_level, data_dir:passdir, platform: config.platform, use_cqhttp_notice: config.use_cqhttp_notice, sign_api_addr:config.sign_api_addr});
...
async function onHttpReq(req, res) {
    ...
    if (req.method === "GET") {
        ...
        try {
            let ret = await api.apply({action, params});
            ret = JSON.parse(ret);
            if(ret.code == null ){
                if(ret.self_id == null) {
                    ret.self_id = account;
