 ...
 */
let bot;
const {fromCqcode} = require("icqq/lib/message/cqCode")
...
async function apply(req) {
    ...
    if (bot[action] && available_actions.includes(action)) {
        ...
        let index = -1;
        if (action === "sendPrivateMsg" || action === "sendGroupMsg" || action === "sendDiscussMsg") {
            index = 1;
        } else if (action === "sendGuildMsg" || action === "sendTempMsg" ) {
            index = 2;
        }

        if (index != -1){
            let message = param_arr[index];
            param_arr[index] = fromCqcode(message);
        }
        ...
        if(ret == true || ret == false){
            ret = {retcode: ret == true ? 0 : 102, status: ret == true ? 'ok' : 'failed',code: ret == true ? 0 : 1, data: {code: ret == true ? 0 : 1}};
        }
